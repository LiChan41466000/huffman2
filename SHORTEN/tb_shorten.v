...
huffman_TM u_huffman(.clk(CLK), .reset(reset), .gray_valid(gray_valid), .gray_data(gray_data),
...
//assign CNT_G = {CNT[1], CNT[2], CNT[3], CNT[4], CNT[5], CNT[6]};//modify here
assign CNT_EXP = {exp_mem[0], exp_mem[1], exp_mem[2], exp_mem[3], exp_mem[4], exp_mem[5]};
//assign HC_G = {HC[1], HC[2], HC[3], HC[4], HC[5], HC[6]};//modify here
assign HC_EXP = {exp_mem[6], exp_mem[7], exp_mem[8], exp_mem[9], exp_mem[10], exp_mem[11]};
//assign M_G = {M[1], M[2], M[3], M[4], M[5], M[6]};//modify here
...
always @(negedge CLK) begin
  if(reset) begin
    ...
    if(DT_O_buf == 2'b01) begin
      CNT_G <= {CNT_G[46:0],CNT_O};
    end
  end
end
//HC_G
...
initial begin
  ...
  #500000; // 根據模擬情況調整這個時間，例如 50,000ns
